...
/** Tests real draft transactions and Layout Builder sections with isolated entities. */
final class PageDraftServiceTest extends TestCase {
  ...
  protected function setUp(): void {
    ...
    // The builder reads the display's render array; isolate rendering, not JSON conversion.
    $display->method('build')->willReturnCallback(function (NodeInterface $node) use ($storages) {
      $sections = [];
      foreach ($node->get('layout_builder__layout')->getSections() as $section) {
        $components = [];
        foreach ($section->getComponents() as $component) {
          $configuration = $component->toArray()['configuration'];
          $matches = $storages['block_content']->loadByProperties([
            'uuid' => substr($configuration['id'], 14),
          ]);
          $components[$component->getUuid()] = ['content' => [
            '#entity_type' => 'block_content', '#block_content' => reset($matches),
          ]];
        }
        $sections[] = ['content' => $components];
      }
      return ['_layout_builder' => $sections];
    });
  ...
  private function entity(string $type, array $values): NodeInterface|BlockContentInterface {
    ...
    $entity->method('hasField')->with('layout_builder__layout')->willReturnCallback(fn() => $this->hasLayout);
    if ($type === 'node') {
      $field = $this->createMock(LayoutSectionItemList::class);
      $field->method('isEmpty')->willReturnCallback(
        fn() => empty($this->created[$id]['values']['layout_builder__layout']));
      $field->method('getSections')->willReturnCallback(
        fn() => $this->created[$id]['values']['layout_builder__layout'] ?? []);
      $entity->method('get')->with('layout_builder__layout')->willReturn($field);
    }
    else {
  ...
  public function testCapabilityWithoutPanelizerDoesNotWrite(): void {
  ...
  public function testCreatesUnpublishedLayoutBuilderPageWithPinnedBlockRevision(): void {
    $input = $this->input();
    ...
    self::assertCount(1, $node['layout_builder__layout']);
    $section = $node['layout_builder__layout'][0];
    self::assertSame('layout_onecol', $section->getLayoutId());
    $components = $section->getComponents();
    self::assertCount(1, $components);
    $component = reset($components);
    self::assertSame('content', $component->getRegion());
    self::assertSame(0, $component->getWeight());
    $configuration = $component->toArray()['configuration'];
  ...
  public function testDraftRoundTripsThroughBuilderPageJson(array $body): void {
    $input = (object) ['title' => 'Drupal 11 草稿测试', 'body' => json_decode(json_encode($body))];
    $drafts = $this->drafts();
  ...
  public function testPartialEntityWritesAndReservationRollBackTogether(string $failure): void {
    ...
    if ($failure === 'layout') {
      ...
      $uuid->method('generate')->willThrowException(new \RuntimeException('Injected layout failure'));
      $this->services['uuid'] = $uuid;
    }
    elseif ($failure === 'node') {
