...
final class PageDraftRegression {
  ...
  public function run(): void {
    ...
    $this->check(count($sections) === 1, 'Draft uses a single Layout Builder section');
    $components = array_values($sections[0]->getComponents());
    $configs = array_map(fn($component) => $component->toArray()['configuration'], $components);
    ...
    $this->check(array_map(fn($component) => $component->getWeight(), $components) ===
      array_keys($input['body']), 'Layout Builder preserves the approved component order');
  ...
  private function rollback(array $input): void {
    $uuid = new class extends \Drupal\Component\Uuid\Php {
    ...
    $drafts = new \Drupal\xinshi_api\PageDraftService(
      ...
      \Drupal::languageManager(), \Drupal::time(), $uuid,
